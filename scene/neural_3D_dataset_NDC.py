    ...
    if not os.path.exists(image_path):
        ...
        while video_frames.isOpened():
            ...
            if ret:
                ...
                if os.path.exists(os.path.join(image_path, "0000.png" % count)):
...
class Neural3D_NDC_Dataset(Dataset):
    def __init__(
        ...
        if "nvidia" in datadir and "dynlbar" in datadir:
            self.img_wh = (
                int(1920 / downsample),
                int(1012 / downsample),
            )
        elif "nvidia" in datadir and "neu3" in datadir:
            self.img_wh = (
                int(960 / downsample),
                int(540 / downsample),
            )
        elif "meetroom" in datadir:
            self.img_wh = (
                int(1222 / downsample),
                int(684 / downsample),
            )
        ...
        self.split = split
        if "nvidia" in datadir:
            self.downsample = 1920 / self.img_wh[0]
        elif "meetroom" in datadir:
            self.downsample = 1222 / self.img_wh[0]
        elif "dynlbar" in datadir:
            self.downsample = 1920 / self.img_wh[0]
        else:
    ...
    def load_meta(self):
        ...
        if "nvidia" in self.root_dir and "dynlbar" in self.root_dir: #
            poses_arr = poses_arr[:12,:]
        ...
        if "nvidia" in self.root_dir:
            N_views = 12
        elif "dynlbar" in self.root_dir:
            N_views = 151
        else:
            N_views = 300
        ...
        self.std_frames = []
        self.calc_std(
            videos, os.path.join(self.root_dir, f"stds_{int(self.downsample)}"), n_frame=N_views
        )  #
    ...
    def __getitem__(self,index):
        ...
        std_index = index // 300 if index // 300 < self.eval_index else index // 300 + 1

        if "nvidia" in self.root_dir:
            return (
                img,
                self.image_poses[index],
                self.image_times[index],
                self.std_frames[index // 12 + 1],
            )
        elif "dynlbar" in self.root_dir:
            return (
                img,
            ...
            return (
                ...
                self.image_poses[index],
                self.image_times[index],
                self.std_frames[std_index] if self.std_frames else None,
                # self.std_frames[index // 300 + 1],
            )  # full
        # return img, self.image_poses[index], self.image_times[index] , self.std_frames[index//100+1] # del
