...
def process_videos(videos, skip_index, img_wh, downsample, transform, num_workers=1):
    ...
    """
    all_imgs = torch.zeros(len(videos) - 1, 12, img_wh[-1] , img_wh[-2], 3)
...
class NVIDIA_NDC_Dataset(Dataset):
    ...
    def calc_std(self, video_path_root, std_path_root, frame_start=0, n_frame=12):
        ...
        for i , video_path in enumerate(video_path_root):
            if self.split == 'train' and i == self.eval_index:
                print(f"跳过 eval 视频: {video_path}")
                continue  # 跳过验证视频，不参与标准差计算
    ...
    def __getitem__(self,index):
        ...
        return img, self.image_poses[index], self.image_times[index] , self.std_frames[index//12+1] 
