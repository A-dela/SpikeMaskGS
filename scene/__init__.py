...
from scene.gaussian_model import GaussianModel,SpikingNeuron
...
class Scene:
    ...
    def __init__(self, args : ModelParams, gaussians : GaussianModel, load_iteration=None, shuffle=True, resolution_scales=[1.0], load_coarse=False):
        ...
        if os.path.exists(os.path.join(args.source_path, "sparse")):
            scene_info = sceneLoadTypeCallbacks["Colmap"](args.source_path, args.images, args.eval, args.llffhold, args.eval_index)
            dataset_type="colmap"
            print("Found Colmap sparse folder, assuming Colmap data set!")
        ...
        elif os.path.exists(os.path.join(args.source_path, "poses_bounds.npy")) and os.path.exists(os.path.join(args.source_path, "readme_for_calibration_file.txt")):
            scene_info = sceneLoadTypeCallbacks["nvidia"](args.source_path, args.white_background, args.eval)
            dataset_type="nvidia"
        elif os.path.exists(os.path.join(args.source_path, "poses_bounds.npy")):
            scene_info = sceneLoadTypeCallbacks["dynerf"](args.source_path, args.white_background, args.eval, args.eval_index)
            dataset_type="dynerf"
