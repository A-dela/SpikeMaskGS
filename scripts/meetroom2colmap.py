...
poses_arr = np.load(os.path.join(root_dir, "poses_bounds.npy"))  # [13,17] 
poses = poses_arr[:, :-2].reshape([-1, 3, 5])  # (N_cams, 3, 5)
near_fars = poses_arr[:, -2:]
videos = glob.glob(os.path.join(root_dir, "cam0[0-9]"))
videos = videos + glob.glob(os.path.join(root_dir, "cam1[0-9]"))
videos = sorted(videos)
assert len(videos) == poses_arr.shape[0]
H, W, focal = poses[0, :, -1]
# focal = focal /2
focal = [focal, focal]
poses = np.concatenate([poses[..., 1:2], -poses[..., :1], poses[..., 2:4]], -1)  # [13,3,4]
...
for index, video_path in enumerate(videos):
    if os.path.exists(os.path.join(video_path,"images","0000.png")):
        ...
        image_path = os.path.join(video_path,"images","0000.jpg")
...
print(1,"SIMPLE_PINHOLE", 1222 , 684 ,  focal[0], 1222 , 684 ,  file=object_cameras_file )
